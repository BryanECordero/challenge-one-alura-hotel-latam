...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblCheckIn.setBounds(68, 136, 350, 14);
		lblCheckIn.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblCheckIn);

		JLabel lblCheckOut = new JLabel("FECHA DE CHECK OUT");
		lblCheckOut.setForeground(SystemColor.textInactiveText);
		lblCheckOut.setBounds(68, 221, 350, 14);
		lblCheckOut.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblCheckOut);

		JLabel lblFormaPago = new JLabel("FORMA DE PAGO");
		lblFormaPago.setForeground(SystemColor.textInactiveText);
		lblFormaPago.setBounds(68, 382, 350, 24);
		lblFormaPago.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblFormaPago);

		JLabel lblTitulo = new JLabel("SISTEMA DE RESERVAS");
		lblTitulo.setBounds(109, 60, 350, 42);
		...
		lblValor.setBounds(72, 303, 350, 14);
		...
		txtFechaSalida.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				if (fechaEntrada != null && fechaSalida != null) {
					long diferenciamilis = fechaSalida.getTime() - fechaEntrada.getTime();
					long dias = TimeUnit.DAYS.convert(diferenciamilis, TimeUnit.MILLISECONDS);
					valor = 1000 * dias;
					fechaIn = fechaEntrada.toInstant().atZone(ZoneId.of("America/Chicago")).toLocalDate();
					fechaOut = fechaSalida.toInstant().atZone(ZoneId.of("America/Chicago")).toLocalDate();
					if (dias < 1) {
						JOptionPane.showMessageDialog(null, "Elija fechas válidas", "Error", JOptionPane.ERROR_MESSAGE);
					} else {
						txtValor.setText(String.format("$ %.2f MXN", valor));
					}
				}
			}
		});
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null) {
					formaDePago = (String) txtFormaPago.getSelectedItem();
					Reserva reserva = new Reserva(fechaIn, fechaOut, valor, formaDePago);
					if (reservaDao.guardar(reserva)) {
						RegistroHuesped registro = new RegistroHuesped(reserva);
						registro.setVisible(true);
						dispose();
					} else {
						JOptionPane.showMessageDialog(contentPane, "No se pudo guardar la reserva, intente nuevamente");
					}
				} else {
					JOptionPane.showMessageDialog(contentPane, "Debe llenar todos los campos.");
				}
			}
		});
