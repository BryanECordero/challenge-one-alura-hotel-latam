...
public class Busqueda extends JFrame {
			...
			public void mouseClicked(MouseEvent e) {
				if (panel.getSelectedIndex() == 0) {
					...
					if (!txtBuscar.getText().isEmpty()) {
						try {
							id = Integer.valueOf(txtBuscar.getText());
						} catch (Exception ex) {
							JOptionPane.showMessageDialog(contentPane, "Escriba un id válido!");
							id = -1;
						}
					}
					if (id >= 0) {
						List<Reserva> lista = reservaDao.buscar(id);
						listarReservas(lista);
					}

				} else {
					List<Huesped> lista = huespedDao.buscar(txtBuscar.getText());
					listarHuspedes(lista);
				}
			}
		});
					...
					Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
							.ifPresentOrElse(fila -> {
								Reserva reserva = new Reserva(
										...
										LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 1).toString()),
										LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 2).toString()),
								...
								int cantidadModificada = reservaDao.modificar(reserva);
								JOptionPane.showMessageDialog(contentPane,
										cantidadModificada + " registro modificado con éxito!");
							}, () -> JOptionPane.showMessageDialog(contentPane, "Por favor, elije un registro"));
				} else {
					...
					Optional.ofNullable(
							...
							.ifPresentOrElse(fila -> {
								Huesped huesped = new Huesped(
										...
										Integer.valueOf(
												modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), 6).toString()));

								int cantidadModificada = huespedDao.modificar(huesped);
								JOptionPane.showMessageDialog(contentPane,
										cantidadModificada + " registro modificado con éxito!");
							}, () -> JOptionPane.showMessageDialog(contentPane, "Por favor, elije un registro"));
				...
				if (panel.getSelectedIndex() == 0) {
					...
					Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
							.ifPresentOrElse(fila -> {
								...
								int cantidadEliminada = reservaDao.eliminar(id);
								modelo.removeRow(tbReservas.getSelectedRow());
								JOptionPane.showMessageDialog(contentPane,
										cantidadEliminada + " registro eliminado con éxito!");
							}, () -> JOptionPane.showMessageDialog(contentPane, "Por favor, elije un registro"));
				} else {
					if (tieneFilaElegida(tbHuespedes)) {
						JOptionPane.showMessageDialog(contentPane, "Por favor, elije un registro");
					...
					Optional.ofNullable(
							...
							.ifPresentOrElse(fila -> {
								...
								int cantidadEliminada = huespedDao.eliminar(id);
	...
	private void listarHuspedes(List<Huesped> huespedes) {
		...
		if (huespedes.size() != 0) {
			...
			JOptionPane.showMessageDialog(contentPane, "Ningún registro encontrado");
		}
	}
	...
	private boolean tieneFilaElegida(JTable tbReservas) {
